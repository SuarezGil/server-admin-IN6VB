...
export const deleteFileOnError= async (err, req, res, next)=>{
    try {
        if (req.file) {
            ...
            if (publicId) {
                await cloudinary.uploader.destroy(publicId);
                console.log(`Archivo Cloudinary eliminado por error en cadena: ${publicId}`)
            }
        }
    } catch (error) {
    console.error(`Error al eliminar archivo de cloudinary (error handler): ${unlinkErr.message}`)
